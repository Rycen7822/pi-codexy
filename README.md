...
- **鼠标点击展开**：面板默认显示**三行任务**，超出时尾部折叠成 `+N more (a completed, b pending)`
  ...
  `ctrl+shift+t` 是同一动作的键盘等价物。**右键单击面板任意一行则手动隐藏**（隐藏态写盘，
  重启仍隐藏）；再打开一次 `/todos` 即恢复显示。首帧锁定高度、实时更新不会缩行（防终端跳动），
