...
- **紧凑产品 footer**：`目录 (分支) +新增 -删除   N tok/s · ↑input ↓output · cache 命中率 · Codex 5h 82% · week 64%`，宽屏追加 `R读 W写`；分支右侧的 `+A -D` 是**工作区相对 HEAD 的改动行数**（含未被 ignore 的未跟踪文件，用与 diff 相同的绿/红，干净仓库不显示）；`N tok/s` 是**实测输出速度**，位于右块最左、紧跟其后才是 `↑input`。窄屏按 优先级 降级（R/W → 缩短目录 → 两行），P0/P1 永不整块消失；不与 metadata 重复显示 model/context。
