...
import { CODEX_REASONING_UPDATE_TYPE, readCodexReasoningUpdate } from "./reasoning-updates.js";
export function projectCodexReasoningEntry(entry) {
    if (entry.type !== "custom" || entry.customType !== CODEX_REASONING_UPDATE_TYPE)
        return entry;
...
export function projectCodexReasoningHistory(entries, messages, leafId) {
    const retired = new Set();
    const byId = new Map(entries.map((entry) => [entry.id, entry]));
    let ancestor = leafId === null ? undefined : leafId ? byId.get(leafId) : entries.at(-1);
    let compacted = false;
    while (ancestor) {
        if (ancestor.type === "compaction")
            compacted = true;
    ...
    const survives = (message) => message.role !== "custom" || message.customType !== CODEX_REASONING_UPDATE_TYPE
    ...
    const projectedEntries = entries.map((entry) => {
        ...
        if (projected !== entry && projected.type === "custom_message" && !retired.has(readCodexReasoningUpdate(projected.details).id))
            virtualIds.add(readCodexReasoningUpdate(projected.details).id);
        return projected;
    });
    if (messages && virtualIds.size === 0)
        return [...messages];
