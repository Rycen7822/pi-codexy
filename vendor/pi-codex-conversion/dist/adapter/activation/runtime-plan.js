import { supportsCodexReasoningUpdates } from "../reasoning-updates.js";
...
import { isCodexLikeModel, isCodexTransportContext, isOpenAIResponsesContext, isResponsesContext } from "../prompt/codex-model.js";
import { APPLY_PATCH_TOOL_NAME, CODE_MODE_TOOL_NAMES, CORE_ADAPTER_TOOL_NAMES, NOTEBOOK_MODE_TOOL_NAMES, SHELL_ADAPTER_TOOL_NAMES, VIEW_IMAGE_TOOL_NAME, CONTEXT_DIRECT_TOOL_NAMES, CONTEXT_MANAGEMENT_TOOL_NAMES, } from "./tool-set.js";
...
function proxySupportsResponsesLite(ctx, config) {
    if (!config.openai.proxyResponsesLite || !configuredProvider(ctx, config) || !isOpenAIResponsesContext(ctx))
        return false;
...
function usesResponsesLite(ctx, config) {
...
function extraToolNames(ctx, config) {
    const names = [];
...
function normalToolNames(ctx, config, contextManagement) {
    const names = [...CORE_ADAPTER_TOOL_NAMES];
    if (supportsViewImageInputs(ctx.model) || config.tools.viewImageFallback)
        names.push(VIEW_IMAGE_TOOL_NAME);
    if (contextManagement)
        names.push(...CONTEXT_MANAGEMENT_TOOL_NAMES);
    return names;
}
export function resolveCodexRuntimePlan(ctx, config, executionMode) {
...
export function resolveCodexRuntimePlanForState(ctx, state) {
...
export function isCodeModeRuntime(plan) {
