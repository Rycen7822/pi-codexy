...
import { CODEX_DEVELOPER_MESSAGE_TYPE, customDeveloperMessageMetadata, isCodexDeveloperMessageDetails, } from "../developer-messages.js";
import { CODEX_CONTEXT_WINDOW_MESSAGE_TYPE, isContextWindowBoundary, rewriteContextWindowGuidance } from "../context-management/messages.js";
import { CODEX_REASONING_UPDATE_TYPE, codexReasoningLane, normalizeCodexConfigurationUpdates, readCodexReasoningUpdate, supportsCodexReasoningUpdates } from "./reasoning-updates.js";
/** Authenticated carrier through Pi's custom-message-to-user conversion. */
export class CodexDeveloperMessageBridge {
    secret = randomBytes(32);
    carriers = new Map();
    contextWindowCarriers = new Set();
    prepare(messages, active, model) {
        const seen = new Set();
        const projected = [];
        for (const message of messages) {
            ...
            if (reasoningUpdate) {
                ...
                if (value.lane !== codexReasoningLane(model))
                    continue;
                id = value.id;
            }
            else {
            ...
            if (isContextWindowBoundary(message))
                this.contextWindowCarriers.add(marker);
            projected.push({ ...message, content: marker });
        }
        return projected;
    }
    rewritePayload(payload, model) {
        if (this.carriers.size === 0)
            return payload;
        if (!isRecord(payload) || !Array.isArray(payload["input"])) {
            if (!containsCarrier(payload, this.carriers))
                return payload;
            throw new Error("Codex developer messages require a Responses input array");
        }
        ...
        if (containsCarrier(input, this.carriers))
            throw new Error("Codex developer message carrier reached an unsupported Responses shape");
        if (!initialEffort)
...
function readCarrierMarker(value) {
...
function toDeveloperMessage(value, content) {
    if (!isRecord(value))
        return value;
...
function containsCarrier(value, carriers) {
    if (typeof value === "string")
        return carriers.has(value);
