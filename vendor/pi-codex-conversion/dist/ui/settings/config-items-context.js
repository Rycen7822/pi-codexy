import { normalizeV2UserMessageRetention, V2_USER_MESSAGE_RETENTION_OPTIONS, } from "../../adapter/activation/config.js";
import { setting, toggle } from "./config-items-shared.js";
export function buildContextSettings(config) {
    return [
        setting({
            ...
            compaction: {
                ...current.compaction,
                contextManagement: value === "remote (encrypted)" ? "remote"
        ...
        ...(config.compaction.contextManagement === "off" ? [] : [setting({ id: "hybridCompaction", label: "Hybrid compaction",
                currentValue: config.compaction.hybridCompaction ? "on" : "off", values: ["off", "on"],
                description: "Allow standard compaction on overflow, rollover or /compact: V2 where supported, Pi summary elsewhere." }, (value, current) => ({
                ...current,
                compaction: { ...current.compaction, hybridCompaction: value === "on" },
            }))]),
        ...(config.compaction.contextManagement === "off" ? [toggle("responsesCompaction", "Responses compaction V2 (Codex)", config.compaction.responsesCompaction, (enabled, current) => ({
                ...current,
                compaction: {
                    ...current.compaction,
            ...
            toggle("portableSummary", "Parallel Pi summary", config.compaction.portableSummary, (enabled, current) => ({
                ...current,
                compaction: {
                    ...current.compaction,
        ...
        ...(config.compaction.responsesCompaction || config.compaction.hybridCompaction ? [setting({
                ...
                values: V2_USER_MESSAGE_RETENTION_OPTIONS.map((value) => `${value}k${value === 64 ? " (Codex native)" : ""}`),
            }, (value, current) => ({
                ...current,
                compaction: {
                    ...current.compaction,
                    v2UserMessageRetention: normalizeV2UserMessageRetention(Number.parseInt(value, 10)) ?? 64,
                },
            }))] : []),
    ];
}
