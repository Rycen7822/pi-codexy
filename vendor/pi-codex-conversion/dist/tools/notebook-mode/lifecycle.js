...
import { NotebookProfileController } from "./profile-lifecycle.js";
...
export class NotebookLifecycleController {
    ...
    async control(request, context, signal) {
        if (request.action === "list")
            return this.profiles.list(request.query);
        if (request.action === "diagnostics")
            return this.host.diagnostics(context, signal);
        if (request.action === "reset")
            return this.host.reset(context, signal);
        if (request.action === "restart" && this.host.runtimeHealth().state !== "ready")
    ...
    async disposeAll(signal) {
        ...
        if (!kernel || this.host.activeCellId())
            return undefined;
    ...
    async release(names, context, signal, preservedNames = []) {
        const activeCell = this.host.activeCellId();
        if (activeCell)
        ...
        const status = parseNotebookRuntimeResult(await kernel.execute(notebookStatusSource(names, statusMarker), { signal }), statusMarker);
    ...
    async prune(query, context, signal) {
        const kernel = this.host.kernel();
    ...
    async restart(context, signal) {
        ...
        if (!activeCell && this.host.runtimeHealth().state === "ready") {
            try {
                ...
                if (this.host.runtimeHealth().state !== "invalidated")
                    throw error;
    ...
    async userBindingNames(kernel, signal) {
