...
import { createJupyterConnectionFile } from "./jupyter-connection.js";
import { diagnoseDenoSyntax } from "./deno-syntax-diagnostics.js";
import { JupyterSocket } from "./jupyter-socket.js";
import { applyExecuteReplyError, applyKernelOutput, finishKernelExecution, } from "./jupyter-output.js";
import { createJupyterMessage, decodeJupyterMessage, encodeJupyterMessage, } from "./jupyter-wire.js";
...
export class DenoJupyterKernel {
    ...
    async execute(code, options = {}) {
        ...
        try {
            const completionState = completion.then((result) => ({ kind: "completion", result }));
            const replyState = this.sendShellRequest(message, undefined, "execute_request")
                .then((reply) => ({ kind: "reply", reply }));
            let result;
            let reply;
            try {
                ...
                if (first.kind === "completion") {
                    result = first.result;
                    reply = (await withTimeout(replyState, REQUEST_TIMEOUT_MS, () => new Error(`Deno Jupyter did not answer execute_request after the cell became idle within ${REQUEST_TIMEOUT_MS}ms`))).reply;
                }
                else {
                    reply = first.reply;
                    result = (await withTimeout(completionState, REQUEST_TIMEOUT_MS, () => new Error(`Deno Jupyter did not become idle after execute_reply within ${REQUEST_TIMEOUT_MS}ms`))).result;
                }
            }
            catch (error) {
    ...
    async complete(code = "", cursorPosition = code.length, signal) {
        await this.start(signal);
        signal?.throwIfAborted();
        if (this.active)
            throw new Error("Cannot request notebook completions while a cell is active");
        ...
        return Array.isArray(matches)
            ? matches.filter((value) => typeof value === "string")
            : [];
    }
    async interrupt() {
    ...
    async shutdown() {
        ...
        const pumps = [this.shellPump, this.iopubPump].filter((pump) => Boolean(pump));
        if (this.control && this.connection) {
            try {
                const message = createJupyterMessage("shutdown_request", { restart: false }, this.session);
                await withTimeout(this.control.send(encodeJupyterMessage(message, this.connection.key)), SHUTDOWN_GRACE_MS, () => new Error(`Deno Jupyter could not send shutdown_request within ${SHUTDOWN_GRACE_MS}ms`));
            }
            catch {
        ...
        this.dispose();
        await withTimeout(Promise.allSettled(pumps), SHUTDOWN_GRACE_MS, () => new Error(`Deno Jupyter socket pumps did not stop within ${SHUTDOWN_GRACE_MS}ms`)).catch(() => undefined);
        this.shellPump = undefined;
        this.iopubPump = undefined;
    }
    async startInner(signal) {
        ...
        let markIopubReady;
        const iopubReady = new Promise((resolve) => { markIopubReady = resolve; });
        this.iopubPump = this.runIopubPump(markIopubReady);
        await this.waitForKernelReady(iopubReady, signal);
    }
    async waitForKernelReady(iopubReady, signal) {
    ...
    async shellRequest(type, content, timeoutMs = REQUEST_TIMEOUT_MS, signal) {
    ...
    async sendShellRequest(request, timeoutMs, requestType = "request", signal) {
        const shell = this.shell;
        const connection = this.connection;
        if (!shell || !connection)
            throw new Error("Deno Jupyter shell is not connected");
    ...
    async runShellPump(socket, connection) {
        try {
            for await (const frames of socket) {
                const message = decodeJupyterMessage([...frames], connection.key);
...
async function withTimeout(pending, timeoutMs, timeoutError) {
    let timer;
