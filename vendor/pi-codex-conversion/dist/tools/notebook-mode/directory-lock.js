...
import { mkdirSync, lstatSync, readFileSync, readdirSync, rmdirSync, unlinkSync, utimesSync, writeFileSync, } from "node:fs";
import { join } from "node:path";
import { setTimeout as delay } from "node:timers/promises";
const OWNER_SUFFIX = ".owner";
export async function acquireDirectoryLock(path, options) {
...
function reclaimStaleDirectoryLock(path, staleMs) {
    try {
        ...
        if (observedOwners.some((owner) => ownerIsFreshOrLive(join(path, owner), staleMs)))
            return;
...
function ownerIsFreshOrLive(path, staleMs) {
    try {
        if (Date.now() - lstatSync(path).mtimeMs <= staleMs)
            return true;
...
function releaseDirectoryLock(path, owner) {
    try {
        unlinkSync(join(path, owner));
    }
    catch { }
