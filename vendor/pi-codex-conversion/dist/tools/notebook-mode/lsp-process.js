...
export class OneShotLspProcess {
    ...
    constructor(options) {
        ...
        this.child.once("exit", (code, signal) => {
            if (!this.stopped)
                this.fail(new Error(`Deno LSP exited unexpectedly (code=${code}, signal=${signal})${this.stderr ? `\n${this.stderr}` : ""}`));
        });
    ...
    request(method, params) {
        if (this.stopped)
            return Promise.reject(new Error("Deno LSP is not running"));
    ...
    readMessage() {
        ...
        if (headerEnd === -1) {
            if (this.input.length > MAX_HEADER_BYTES)
                throw new Error("Deno LSP returned an oversized header");
            return false;
        }
        ...
        if (!match)
            throw new Error("Deno LSP returned a message without Content-Length");
        ...
        if (this.input.length < bodyStart + length)
            return false;
        ...
        if (!isRecord(message) || message["jsonrpc"] !== "2.0")
            throw new Error("Deno LSP returned invalid JSON-RPC");
        this.handle(message);
        return true;
    }
    handle(message) {
