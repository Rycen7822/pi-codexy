...
export class JupyterSocket {
    ...
    async connect(signal) {
        ...
        try {
            ...
            if (handshake.flags !== 4 && handshake.flags !== 6)
                throw new Error("Expected Jupyter ZMTP READY command");
            validateReady(handshake.body, this.type);
            if (this.type === "SUB")
                await this.write(Buffer.from([0, 1, 1]));
            this.ready = true;
        }
        catch (error) {
    ...
    async send(frames) {
        if (!this.ready || this.type !== "DEALER")
            throw new Error("Jupyter socket is not ready to send");
        ...
        try {
            while (!this.stopped.signal.aborted) {
                ...
                while (true) {
                    ...
                    if (frame.flags & 4)
                        throw new Error("Unexpected Jupyter ZMTP command");
                    frames.push(frame.body);
                    size += frame.body.length;
                    if (frames.length > MAX_FRAMES)
                        throw new Error("Jupyter multipart frame limit exceeded");
                    if (!(frame.flags & 1))
                        break;
                }
                yield frames;
    ...
    async read(size) {
        ...
        while (offset < size) {
            if (!this.buffered.length) {
                ...
                if (chunk.done)
                    throw new Error("Jupyter socket disconnected");
    ...
    async readFrame(limit = MAX_MESSAGE_BYTES) {
        const flags = (await this.read(1))[0];
        if (flags & 0xf8 || ((flags & 4) && (flags & 1)))
            throw new Error("Invalid Jupyter ZMTP frame flags");
...
function encodeFrame(body, flags) {
...
function validateReady(body, type) {
    ...
    for (let offset = 6; offset < body.length;) {
        ...
        if (!nameLength || offset + nameLength + 4 > body.length)
            throw new Error("Invalid Jupyter READY metadata");
        ...
        if (offset + valueLength > body.length)
            throw new Error("Invalid Jupyter READY metadata");
        if (name === "socket-type")
            peerType = body.toString("ascii", offset, offset + valueLength);
        offset += valueLength;
    }
    if (peerType !== (type === "DEALER" ? "ROUTER" : "PUB"))
        throw new Error("Incompatible Jupyter ZMTP socket type");
}
