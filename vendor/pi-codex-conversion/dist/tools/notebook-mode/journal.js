...
import { applyNotebookJournalEvent, emptyNotebookDocument, notebookCellId, notebookCellStatus, readNotebookDocument, writeNotebookDocument, } from "./journal-document.js";
const MAX_CELL_OUTPUT_CHARS = 16 * 1024 * 1024;
const JOURNAL_DOCUMENT_OVERHEAD_BYTES = 4_096;
export function initializeNotebookJournal(identity, maxBytes) {
    ...
    for (const candidate of [path, eventsPath, previousPath]) {
        if (fileSize(candidate) > maxBytes)
            rmSync(candidate, { force: true });
    }
    if (!existsSync(path))
        writeNotebookDocument(path, emptyNotebookDocument(project, identity.session), maxBytes);
...
export function beginNotebookJournalCell(journal, cell) {
    if (!journal.writable)
        throw new Error(`Notebook journal requires diagnostics: ${journal.path}`);
    appendEvent(journal, { type: "begin", id: cell.id, source: cell.source, createdAt: new Date().toISOString() });
    journal.cells += 1;
}
function rotateNotebookJournalIfNeeded(journal, incomingBytes) {
    if (fileSize(journal.path) + fileSize(journal.eventsPath) + incomingBytes <= journal.maxBytes)
        return false;
    materializeNotebookJournal(journal);
...
export function finishNotebookJournalCell(journal, cell) {
    if (!journal.writable)
        throw new Error(`Notebook journal requires diagnostics: ${journal.path}`);
    const rotated = appendEvent(journal, {
        ...
        outputs: journalOutputs(cell.items, cell.result, Math.min(MAX_CELL_OUTPUT_CHARS, Math.max(0, Math.floor((journal.maxBytes - Buffer.byteLength(cell.source) - JOURNAL_DOCUMENT_OVERHEAD_BYTES) / 6)))),
    });
    if (rotated)
        journal.cells = 1;
    journal.completedCells += 1;
}
export function materializeNotebookJournal(journal) {
    const document = readNotebookDocument(journal.path, journal.maxBytes);
    if (!document)
        throw new Error(`Notebook journal is invalid: ${journal.path}`);
    const events = readEvents(journal.eventsPath, journal.maxBytes);
    if (events.length === 0)
        return;
    for (const event of events)
        applyNotebookJournalEvent(document, event);
...
export function readNotebookJournalCodeCells(path) {
...
function materializedDocument(path) {
    const document = readNotebookDocument(path);
    if (!document)
        throw new Error(`Notebook journal is invalid: ${path}`);
    for (const event of readEvents(journalEventsPath(path)))
        applyNotebookJournalEvent(document, event);
    return document;
}
...
function readEvents(path, maxBytes) {
    if (!existsSync(path))
        return [];
    try {
        if (maxBytes !== undefined && fileSize(path) > maxBytes)
            throw new Error("events exceed budget");
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            try {
                ...
                if (!isNotebookJournalEvent(value))
...
function isNotebookJournalEvent(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const event = value;
    if (typeof event["id"] !== "string" || typeof event["source"] !== "string")
        return false;
...
function journalEventsPath(path) {
    return `${path}.events.jsonl`;
}
function previousJournalPath(path) {
    return path.replace(/\.ipynb$/, ".previous.ipynb");
}
function journalOutputs(items, result, maxChars) {
    const outputs = [];
