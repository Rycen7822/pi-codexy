...
export function extractNotebookNpmImports(source) {
    const imports = new Set();
    for (const literal of stringLiterals(source)) {
        if (!isExactNpmSpecifier(literal.value) || Buffer.byteLength(literal.value) > MAX_SPECIFIER_BYTES)
            continue;
        const prefix = literal.maskedPrefix.trimEnd();
        if (/(?:^|[^\w$.])import\s*(?:\(\s*)?$|(?:^|[^\w$])from\s*$/.test(prefix))
            imports.add(literal.value);
    }
    return [...imports].sort();
}
export function readNotebookNpmImports(identity) {
...
export async function recordNotebookNpmImports(identity, imports) {
    if (imports.length === 0)
...
export function formatNotebookNpmImportsNotice(imports) {
...
function npmImportPaths(identity) {
...
function writeManifest(path, manifest) {
    ...
    if (Buffer.byteLength(text) > MAX_FILE_BYTES)
        throw new Error("Notebook npm inventory is too large");
...
function boundedImportList(imports) {
    let output = "";
    let included = 0;
    for (const specifier of imports) {
        const next = output ? `${output}, ${specifier}` : specifier;
        if (Buffer.byteLength(next) > MAX_IMPORT_LIST_BYTES)
            break;
        output = next;
        included += 1;
    }
...
function isNpmImportsManifest(value, project) {
    ...
    return record["schema"] === NPM_IMPORTS_SCHEMA
        ...
        && record["imports"].every((specifier) => typeof specifier === "string"
            && isExactNpmSpecifier(specifier)
            && Buffer.byteLength(specifier) <= MAX_SPECIFIER_BYTES);
}
function isExactNpmSpecifier(specifier) {
    if (!NPM_SPECIFIER.test(specifier))
        return false;
...
function stringLiterals(source) {
    ...
    for (let index = 0; index < source.length;) {
        ...
        for (let cursor = start; cursor < literal.stop; cursor += 1)
            if (source[cursor] !== "\n")
                masked[cursor] = " ";
        index = literal.stop;
...
function readStaticLiteral(source, start, delimiter) {
    let index = start + 1;
    let raw = "";
    let dynamic = false;
    while (index < source.length) {
        ...
        if (delimiter === "`" && char === "$" && source[index + 1] === "{")
            dynamic = true;
        if (delimiter !== "`" && (char === "\n" || char === "\r"))
            dynamic = true;
        raw += char;
...
function decodeStaticLiteral(raw) {
    let value = "";
    for (let index = 0; index < raw.length; index += 1) {
        ...
        if (escaped in simple) {
            if (escaped === "0" && /[0-9]/.test(raw[index + 1] ?? ""))
                return undefined;
        ...
        if (escaped === "u") {
            ...
            if (end < 0 || !/^[0-9a-f]{1,6}$/i.test(hex))
                return undefined;
