...
async function restoreProjectStateLocked(kernel, identity, paths) {
    identity.signal?.throwIfAborted();
    const manifest = readProjectStateManifest(paths.manifest);
    if (!manifest)
        return emptyProjectStateSummary();
...
export async function projectStateBindingSelection(kernel, signal) {
    const marker = `__PI_NOTEBOOK_PROJECT_BINDINGS_${randomUUID()}__`;
    return parseProjectBindingNames(await kernel.execute(projectBindingNamesSource(marker), { signal }), marker);
}
export async function syncProjectStateBindings(kernel, names, signal) {
...
export function formatProjectStateNotice(summary) {
    if (summary.message)
...
function writeMergedProjectState(paths, identity, current, candidate, merged) {
...
function writeProjectConflict(directory, identity, merged) {
...
function listProjectConflicts(directory) {
    const names = new Set();
...
function removeResolvedProjectConflicts(directory, names) {
    if (names.size === 0)
        return;
    const conflicts = join(directory, "conflicts");
    for (const file of readDirectoryNames(conflicts)) {
        ...
        try {
            ...
            if (!record || !record.names.some((name) => names.has(name)))
                continue;
            if (record.payload)
