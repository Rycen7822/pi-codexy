...
export function withinNameBudget(names) {
    let bytes = 0;
...
export function takeDetailValues(values, budget) {
    const selected = [];
...
export function remainingDetailsBudget(base) {
    return { remaining: Math.max(0, NOTEBOOK_DETAILS_BUDGET - Buffer.byteLength(JSON.stringify(base))) };
}
export function boundedReleaseDetails(result, protectedNames, restarted, checkpoint) {
...
export function formatStatus(details) {
    ...
    if (details.query === undefined && details.pinned.length > 0) {
        ...
        for (const binding of details.pinned)
    ...
    if (details.query === undefined && (details.largestUnpinned.length > 0 || details.omittedLargestUnpinned > 0)) {
        ...
        if (details.omittedLargestUnpinned > 0)
            lines.push(`${details.omittedLargestUnpinned} additional unpinned binding(s) omitted; use status with a query glob`);
...
function formatBindingMetadata(binding) {
...
export function formatRelease(result, restarted) {
    const lines = [
