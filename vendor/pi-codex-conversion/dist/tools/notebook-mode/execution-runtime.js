import { formatCodeModeToolHelp } from "../code-mode/custom-tool-prompt.js";
import { CodeModeDelegateRuntime } from "../code-mode/delegate-runtime.js";
import { DEFAULT_CODE_MODE_EXEC_YIELD_MS, isCustomToolDefinition, parseExecSource, } from "../code-mode/host-protocol.js";
import { directToolYieldTime } from "../code-mode/tool-source.js";
import { codeModeGlobalName, codeModeNameForToolIdentity, resolveCodeModeToolIdentity, } from "../code-mode/tool-identity.js";
import { NotebookBridgeServer } from "./bridge-server.js";
import { NotebookCell } from "./cell.js";
import { beginNotebookJournalCell, finishNotebookJournalCell } from "./journal.js";
import { NOTEBOOK_INTERRUPTED_NOTICE } from "./runtime-health.js";
...
export class NotebookExecutionRuntime {
    ...
    async execute(source, context, signal, tools = []) {
        ...
        const outputHints = Object.fromEntries(tools.flatMap((tool) => "textOutput" in tool && tool.textOutput !== undefined
    ...
    async runCell(cell, source, journaled) {
        ...
        try {
            ...
            const normalized = result.errorName === "PiNotebookExit" && result.errorValue === this.bridge.exitToken
                ? { ...result, status: "ok", errorText: undefined, errorName: undefined, errorValue: undefined }
                : result;
    ...
    async observe(cell, yieldTimeMs, signal) {
        signal?.throwIfAborted();
    ...
    async stopAndCloseCell(cell) {
        await this.stopCell(cell);
        this.closeCell(cell);
    }
    async recoverAfterFatal(context) {
    ...
    reportJournalFailure(error, operation, cell) {
