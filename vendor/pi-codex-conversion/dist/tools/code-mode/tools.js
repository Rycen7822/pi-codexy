...
import { discoverCustomToolsFromDirectories, getCustomToolsDir, getProjectCustomToolsDir, } from "./custom-tools.js";
import { replaceCodeModeToolsPrompt } from "./custom-tool-prompt.js";
import { registerPublicCodeModeTools } from "./public-tools.js";
import { SharedCodeModeRuntime, } from "./shared-runtime.js";
...
export async function registerCustomTools(pi, toolsDir, options = {}) {
...
function isTrustedProjectContext(value) {
    return Boolean(value &&
        typeof value === "object" &&
...
export async function registerCodeModeTools(pi, options) {
    ...
    return {
        ...
        refreshPromptTools(systemPrompt, ctx) {
            ...
            const documentationPath = activeProviders.find((provider) => provider.documentationPath)?.documentationPath;
            const previousSection = runtime.getPromptSection();
            const nextTools = runtime.refreshPromptTools(ctx);
            const replacement = replaceCodeModeToolsPrompt(systemPrompt, previousSection, nextTools, documentationPath);
...
async function getOrCreateRuntime(pi) {
    const state = pi.events;
...
function customToolsDocumentationPath() {
