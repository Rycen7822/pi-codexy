...
export function toCodeModeToolResult(response, maxTokens) {
    const scriptError = response.kind === "result" ? withScriptErrorRecovery(response.errorText) : undefined;
    ...
    const output = response.contentItems
        ...
        if (content?.type !== "image")
            return content;
        if (imageCount >= MAX_OUTPUT_IMAGE_COUNT ||
    ...
    const outputTokens = Math.min(MAX_CODE_MODE_OUTPUT_TOKENS, Math.max(1, maxTokens ?? response.maxOutputTokens ?? DEFAULT_CODE_MODE_OUTPUT_TOKENS));
    return {
        content: [
            { type: "text", text: status },
            ...(memoryWarning ? [{ type: "text", text: memoryWarning }] : []),
...
function withScriptErrorRecovery(errorText) {
    if (!errorText || !/Identifier ['"][^'"]+['"] has already been declared/.test(errorText))
        return errorText;
...
export function formatNotebookMemoryWarning(memory) {
...
function truncateTextContent(content, maxChars) {
    ...
    for (const item of content) {
        ...
        if (remaining <= 0) {
            if (!truncated)
                output.push({ ...item, text: "[Output truncated]" });
            truncated = true;
            continue;
        }
