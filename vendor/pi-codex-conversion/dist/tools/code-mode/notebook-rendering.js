import { auxiliaryToolRenderers, displayRecord, inlineToolText, } from "../../ui/tool-rendering/auxiliary-tool.js";
const TITLES = {
...
export const notebookRenderers = auxiliaryToolRenderers("Notebook operation failed", (args, result) => {
    ...
    const count = (value, singular, plural = `${singular}s`) => {
        if (typeof value === "number")
            facts.push(`${value} ${value === 1 ? singular : plural}`);
    };
    switch (action) {
        case "status": {
            if (Array.isArray(details["matches"]))
                count(details["matches"].length + (typeof details["omittedMatches"] === "number" ? details["omittedMatches"] : 0), "match", "matches");
            ...
            if (typeof heap === "number")
                facts.push(`${Math.round(heap / 1024 ** 2)} MiB heap`);
            if (typeof heap === "number" && typeof limit === "number" && limit > 0 && heap / limit >= 0.8)
                warnings.push(`Heap at ${Math.round(heap / limit * 100)}% of limit`);
            if (details["state"] === "running")
                warnings.push(`Cell running${inlineToolText(details["activeCell"]) ? `: ${inlineToolText(details["activeCell"])}` : ""}`);
            if (displayRecord(details["checkpoint"])["dirty"] === true)
                warnings.push("Checkpoint pending");
            break;
        }
        case "list":
            if (Array.isArray(details["profiles"]))
                count(details["profiles"].length, "saved profile");
            break;
        case "checkpoint":
            count(details["projectBindings"], "durable binding");
            break;
        case "pin":
        case "unpin":
            count(details["bindingCount"], "binding");
            break;
        case "release":
        case "prune":
            ...
            if (details["restarted"] === true)
                warnings.push("Kernel restarted · runtime-only handles cleared");
            if (typeof details["failureCount"] === "number" && details["failureCount"] > 0)
                warnings.push(`${details["failureCount"]} cleanup/release failures`);
            break;
        case "save":
        case "load": {
            ...
            if (typeof profile["skipped"] === "number" && profile["skipped"] > 0)
                warnings.push(`${profile["skipped"]} skipped`);
            break;
        }
        case "reset":
        ...
        case "diagnostics":
            ...
            if (displayRecord(details["runtime"])["state"] === "invalidated")
                warnings.push("Runtime invalidated");
            if (typeof details["error"] === "string")
                warnings.push(details["error"]);
            break;
    }
    if (typeof details["terminatedCell"] === "string")
        warnings.push(`Terminated cell ${inlineToolText(details["terminatedCell"])}`);
    if (Array.isArray(details["disposalFailures"]) && details["disposalFailures"].length > 0)
        warnings.push(`${details["disposalFailures"].length} resource cleanup failures`);
    if (typeof details["restoreNotice"] === "string")
        warnings.push(details["restoreNotice"]);
