import { translateCodeModeGuideline, translateCodeModeToolReferences, translateCodeModeUsage, } from "./tool-identity.js";
...
function isDeferredDiscoverableTool(tool) {
...
export function formatCodeModeToolHelp(tool) {
    return [
        ...
        tool.promptSnippet
            ...
            : undefined,
        ...(tool.promptGuidelines ?? []).map((guideline) => translateCodeModeGuideline(guideline, tool.name)),
...
function translatedPromptLines(tool) {
    if (!("invoke" in tool) || tool.translatePromptMetadata !== true)
        return [];
    // Usage owns the callable contract here; native descriptions and schemas do not.
    return (tool.promptGuidelines ?? []).map((guideline) => `- ${translateCodeModeGuideline(guideline, tool.name)}`);
}
function buildGuidanceSection(tools) {
    const lines = tools
