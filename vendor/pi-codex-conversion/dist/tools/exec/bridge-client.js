...
export function createExecBridgeClient(binaryPath = () => getBundledToolBinaryPath("exec_bridge")) {
    ...
    function getBridge() {
        if (bridgeClosed)
            throw new Error("exec_bridge is shut down");
    ...
    async function sendRequest(value, existingChild) {
...
function killBridgeProcessTree(child) {
    const pid = child.pid;
...
function killUnixDescendants(rootPid) {
...
function waitForBridgeClose(child, timeoutMs) {
    if (child.exitCode !== null || child.signalCode !== null)
        return Promise.resolve();
    return withTimeout(new Promise((resolve) => child.once("close", () => resolve())), timeoutMs, "exec_bridge did not exit after shutdown");
}
