...
import { renderWriteStdinCall } from "../../ui/tool-rendering/codex-rendering.js";
...
function parseFormattedExecTranscript(text) {
    const marker = "\nOutput:\n";
...
function parseWriteStdinParams(params) {
    ...
    const max_output_tokens = "max_output_tokens" in params && typeof params.max_output_tokens === "number" ? params.max_output_tokens : undefined;
...
export function createWriteStdinTool(sessions, options = {}) {
    const tool = {
        name: "write_stdin",
        label: "write_stdin",
        ...
        renderResult(result, { expanded, isPartial }, theme) {
            ...
            if (!expanded) {
                if (!isPartial || !options.showOutputWhenCollapsed)
...
export function registerWriteStdinTool(pi, sessions, options = {}) {
    pi.registerTool(createWriteStdinTool(sessions, options));
}
