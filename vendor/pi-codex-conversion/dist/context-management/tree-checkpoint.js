...
export function projectTreeCheckpointBranch(active, all) {
    ...
    if (index.invalidManifest)
        throw new Error("Invalid Tree archive checkpoint metadata");
    const hiddenSummaries = new Set(index.archives.filter((item) => !item.manifest.compactionEntryId || hasTreeArchiveSuccessor(active, item.manifest.windowId))
    ...
    if (tail.some((entry) => entry.type === "compaction"))
        return hideArchiveSummaries(active, hiddenSummaries);
    const byId = new Map(all.map((entry) => [entry.id, entry]));
    const source = [];
    const visited = new Set();
    let id = archive.manifest.archivedLeafId;
    while (id !== null) {
        if (visited.has(id) || source.length >= 50_000)
            throw new Error("Invalid Tree checkpoint ancestry");
        visited.add(id);
        const entry = byId.get(id);
        if (!entry)
            throw new Error("Tree checkpoint ancestry is missing");
        source.push(entry);
        id = entry.parentId;
    }
    source.reverse();
    ...
    return hideArchiveSummaries([...source, ...tail].map((entry, position, entries) => {
        const parentId = position === 0 ? null : entries[position - 1].id;
        return entry.parentId === parentId ? entry : { ...entry, parentId };
    }), hiddenSummaries);
}
function hideArchiveSummaries(entries, hidden) {
    if (hidden.size === 0)
        return entries;
    return entries.map((entry) => entry.type === "branch_summary" && hidden.has(entry.id)
...
export function projectTreeCheckpointMessages(active, projected, messages) {
    if (!messages || active === projected)
        return messages;
