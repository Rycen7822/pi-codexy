...
export const CONTEXT_WINDOW_COMPACTION_SUMMARY = "[Pi Codex context-window boundary; no conversation summary was generated.]";
export const CONTEXT_WINDOW_COMPACTION_STRATEGY = "codex-context-window";
...
export function rewriteContextWindowGuidance(content, astra) {
    return content.replace(/^<context_window_guidance>[\s\S]*?<\/context_window_guidance>/, astra ? CONTEXT_WINDOW_GUIDANCE : CONTEXT_WINDOW_EXPLICIT_GUIDANCE);
}
export function renderContextWindowMessage(identity, threadHint) {
    const lines = [
        "<context_window>",
        "Agent name: /root",
    ...
    if (threadHint)
        lines.push(threadHint);
    lines.push("</context_window>");
    return `${CONTEXT_WINDOW_GUIDANCE}\n\n${lines.join("\n")}`;
}
export function renderContextWindowReminder(remainingTokens) {
...
export function renderManualContextCheckpoint(customInstructions) {
    return `<context_window_reminder>
...
export function isCodexContextManagementMessageDetails(value) {
    if (!value || typeof value !== "object")
        return false;
    const details = value;
    if (details["protocol"] !== 1 ||
...
export function isContextWindowBoundary(message) {
...
export function isContextWindowCompactionDetails(value) {
...
export function sendContextWindowMessage(pi, content, kind, identity, options, trimPreviousWindow = false) {
    pi.sendMessage({
