...
export function historyNotesRenderers(namespace) {
    return auxiliaryToolRenderers(`${namespace === "notes" ? "Notes" : "History"} operation failed`, (args, result) => {
        ...
        if (Array.isArray(entries)) {
            ...
            summary = `${file["bytes"].toLocaleString("en-US")} bytes`;
        else if (typeof data["returned_lines"] === "number")
            summary = `${data["returned_lines"]} lines returned`;
        if (typeof file["start_line"] === "number" && typeof file["stop_line"] === "number")
            summary = `Lines ${file["start_line"]}–${file["stop_line"]} of ${file["total_lines"]}`;
        if (typeof item["content"] === "string")
            summary = `${inlineToolText(item["role"]) ?? "History"} · ${item["content"].length.toLocaleString("en-US")} characters returned${item["next_offset_chars"] === undefined ? "" : " · more available"}`;
        ...
        if (query)
            summary = [JSON.stringify(query), summary].filter(Boolean).join(" · ");
...
export function renderContextWindowBoundary(details, expanded, theme) {
