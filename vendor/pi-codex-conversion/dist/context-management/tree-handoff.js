...
import { createPiSessionNotesSnapshot } from "./local-notes.js";
import { CODEX_CONTEXT_WINDOW_MESSAGE_TYPE } from "./messages.js";
/** Anchor the selected range in conversation content, not private Pi entry IDs. */
function summaryScope(branch, firstSelectedId) {
    const start = branch.findIndex((entry) => entry.id === firstSelectedId);
    if (start < 0)
        throw new Error("The selected conversation range is no longer on the current path");
    for (let index = start - 1; index >= 0; index--) {
        ...
        if (entry.type === "custom_message" && entry.customType === CODEX_CONTEXT_WINDOW_MESSAGE_TYPE) {
            ...
            if (windowId)
                return `since the start of context window ${windowId}`;
        }
        ...
        if (text)
            return `since the ${message.role} message beginning ${JSON.stringify(text.slice(0, 240))}`;
...
export class CodexTreeHandoff {
    ...
    finishNoteWrite(action, path, ctx) {
        const pending = this.pending;
        if (!pending?.started || pending.sessionId !== ctx.sessionManager.getSessionId() ||
            path !== pending.path || (action !== "write_file" && action !== "append_to_file"))
            return false;
        pending.noteWritten = true;
        return true;
    }
    async prepare(pi, event, ctx, mode) {
        if (this.pending) {
        ...
        try {
            ...
            const pending = { sessionId, path, prompt, preparing: false, started: false, noteWritten: false,
            ...
            startupTimer = setTimeout(() => {
                if (!pending.started)
                    completed.reject(new Error("Handoff agent did not start. Check model authentication and retry the jump"));
            }, 30_000);
