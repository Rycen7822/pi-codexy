...
function parserIsDone({ state, prefixes }) {
...
function parserReadStr({ state, prefix, returnEverything, }) {
    if (state.index >= state.lines.length) {
...
export function parsePatchActions({ text }) {
    ...
    while (index < lines.length - 1) {
        ...
        throw new DiffError(`Invalid patch hunk on line ${lineNumber}: '${line}' is not a valid hunk header. Valid hunk headers: ${VALID_HUNK_HEADERS}`);
    }
    if (actions.length === 0) {
