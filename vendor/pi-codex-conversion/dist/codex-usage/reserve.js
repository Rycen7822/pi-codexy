import { isCanonicalCodexSubscriptionModel } from "../adapter/prompt/codex-model.js";
import { isTerminalRateLimitError } from "../providers/openai-codex/errors.js";
import { fetchCodexReserveStatus } from "./client.js";
import { CODEX_RESERVE_MODEL } from "./reserve-policy.js";
const RETURN_ENTRY = "codex-reserve-return";
function readReturn(ctx) {
...
export function createCodexReserveController(pi) {
    let switching = false;
    return {
        modelSelected(ctx) {
            if (!switching && ctx.model?.id !== CODEX_RESERVE_MODEL && readReturn(ctx))
                pi.appendEntry(RETURN_ENTRY, null);
        },
        async beforeTurn(ctx) {
            if (ctx.model?.id !== CODEX_RESERVE_MODEL || !isCanonicalCodexSubscriptionModel(ctx.model))
                return;
            ...
            try {
                ...
                if (ctx.model !== model || ctx.sessionManager.getLeafId() !== leaf)
                    return;
                ...
                if (!await pi.setModel(target))
                    throw new Error("Original model authentication is unavailable");
        ...
        async settled(ctx) {
            ...
            if (last?.type !== "message" || last.message.role !== "assistant" || last.message.stopReason !== "error")
                return false;
            if (last.message.model !== model.id || last.message.provider !== model.provider)
                return false;
            const errorText = last.message.errorMessage ?? "";
            if (!isTerminalRateLimitError(errorText) && !/Codex usage limit reached|hit your ChatGPT usage limit/i.test(errorText))
                return false;
            if (model.id === CODEX_RESERVE_MODEL)
                return true;
            const leaf = ctx.sessionManager.getLeafId();
            try {
                const status = await fetchCodexReserveStatus(ctx);
                if (ctx.model !== model || ctx.sessionManager.getLeafId() !== leaf || !status?.entryAllowed)
                    return true;
                ...
                const previous = { accountKey: status.accountKey, provider: model.provider, modelId: model.id, thinkingLevel: pi.getThinkingLevel() };
                pi.appendEntry(RETURN_ENTRY, previous);
                switching = true;
                if (!await pi.setModel(reserve))
                    throw new Error("Luna Reserve authentication is unavailable");
                pi.setThinkingLevel(previous.thinkingLevel);
