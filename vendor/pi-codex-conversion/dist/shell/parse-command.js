...
import { cdTarget } from "./command-operands.js";
import { isSmallFormattingCommand, summarizeMainTokens, } from "./command-summary.js";
import { joinCommandTokens, joinPaths, normalizeTokens, shellSplit, splitOnConnectors, } from "./tokenize.js";
...
function parseCommandImpl(command) {
    ...
    const parts = containsConnectors(normalized)
        ...
        : [normalized];
    const effectiveParts = parts.length > 1
...
function parseShellLcCommands(original) {
    ...
    if (commands.length === 1) {
        const hadConnectors = hadMultipleCommands ||
