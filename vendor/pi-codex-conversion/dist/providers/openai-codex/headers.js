...
function buildBaseCodexHeaders(modelHeaders, additionalHeaders, accountId, token, originator, routingHint) {
    ...
    if (routingHint)
        ...
        headers.delete(X_CODEX_ROUTING_HINT_HEADER);
    const os = osInfo.current;
    headers.set("User-Agent", os ? `pi (${os.platform()} ${os.release()}; ${os.arch()})` : "pi (browser)");
    return headers;
}
export function buildSSEHeaders(modelHeaders, additionalHeaders, accountId, token, sessionId, responsesLite = false, originator = "pi", routingHint) {
...
export function buildWebSocketHeaders(modelHeaders, additionalHeaders, accountId, token, requestId, originator = "pi", routingHint) {
    const headers = buildBaseCodexHeaders(modelHeaders, additionalHeaders, accountId, token, originator, routingHint);
